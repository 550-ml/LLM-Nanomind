...
# Critic Model 用来估计状态价值，然后reward就是采样这个估计
class CriticModel(NanoMindForCausalLM):
    ...
    def forward(self,input_ids=None, attention_mask=None, **kwargs ):
        ...
        hidden_states = self.model.norm(outputs[0])  # [B, S, D]
...
def ppo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, actor_scheduler, critic_scheduler, reward_model, start_step=0, wandb=None, use_sglang=False):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        with torch.no_grad():  # Rollout阶段只需推理获取old_logp和old_values，切断梯度省显存
            ...
            gen_len = old_resp_values.size(1); lastgaelam = torch.zeros(B, device=args.device); advs_rev = []  # 20, B
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Mind PPO (Proximal Policy Optimization)")
    ...
    parser.add_argument("--wandb_project", type=str, default="NanoMind-PPO", help="wandb项目名")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"NanoMind-PPO-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
