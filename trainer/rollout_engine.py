...
def compute_per_token_logps(model, input_ids: Tensor, n_keep: int, attention_mask: Optional[Tensor] = None) -> Tensor:
    ...
    input_ids = input_ids.detach().clone() if input_ids.is_inference() else input_ids  # [B, S]
    logits = unwrapped(input_ids,attention_mask=attention_mask, logits_to_keep=n_keep + 1).logits[:, :-1, :]  # [B, S, V]
    per_token_logps = []
    # for logits_row, ids_row in zip(logits, input_ids[:-n_keep:]):
    #     # [L-1, V] [n_keep]
    #     ids_row = ids_row.detach().clone() if ids_row.is_inference() else ids_row
    #     torch.gather(logits_row.log_softmax(dim=-1),dim=1, index=ids_row.unsqueeze(1)).squeeze(1)  # [n_keep, 1]
    for logits_row, ids_row in zip(logits, input_ids[:, -n_keep:]):
        ids_row = ids_row.detach().clone() if ids_row.is_inference() else ids_row
        per_token_logps.append(
            torch.gather(logits_row.log_softmax(dim=-1), 1, ids_row.unsqueeze(1)).squeeze(1)
        )
    return torch.stack(per_token_logps)  # [B. nkeep]
...
class TorchRolloutEngine(RolloutEngine):
    def __init__(
        self,
        policy_model: torch.nn.Module, # 生成数据的
        tokenizer,
        device: str = "cuda",
        autocast_ctx = None  # 混合精度上下文
    ):
